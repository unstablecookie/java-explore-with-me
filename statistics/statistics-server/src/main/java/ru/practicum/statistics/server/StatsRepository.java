...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query(value = "SELECT distinct on (e.uri) e.app, e.uri , ee.hits from ( " +
            ...
            " FROM endpointhits WHERE timestamp >= ?1 and timestamp <= ?2 GROUP BY uri ORDER BY hits DESC ) ee " +
            "JOIN endpointhits e on e.uri = ee.uri", nativeQuery = true)
    List<IViewStatsDto> findEndpointHits(LocalDateTime start, LocalDateTime end);

    @Query(value = "SELECT distinct on (ee.uri) e.app, ee.uri, ee.hits FROM ( " +
            "SELECT uri, count(distinct ip) as hits from endpointhits WHERE timestamp >= ?1 and timestamp <= ?2 " +
            "group by uri ORDER BY hits DESC " +
            ") ee JOIN endpointhits e on e.uri = ee.uri ",
            nativeQuery = true)
    List<IViewStatsDto> findEndpointHitsWithUniqueIp(LocalDateTime start, LocalDateTime end);

    @Query(value = "SELECT distinct on (e.uri) e.app, e.uri , ee.hits from ( " +
            " SELECT uri, count(uri) as hits " +
            " FROM endpointhits WHERE uri in ?3 and timestamp >= ?1 and timestamp <= ?2 GROUP BY uri ORDER BY hits DESC ) ee " +
            "JOIN endpointhits e on e.uri = ee.uri", nativeQuery = true)
